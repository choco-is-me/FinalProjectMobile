package com.moh.alarmclock.Date;
...
public class Date implements MoSavable, MoLoadable {
    ...
    public long timeInMilli(){
        return (long)(this.calendar.get(Calendar.HOUR_OF_DAY)* TimeUtils.MILLI_IN_HOUR
                + this.calendar.get(Calendar.MINUTE) * TimeUtils.MILLI_IN_MINUTE
                + this.calendar.get(Calendar.SECOND) * TimeUtils.MILLI_IN_SECOND);
    }

   public float isInTimeRange(Date date, long tolerance){
...
//        }
    ...
    private int[] difference(Calendar c){
        int[] r = new int[5];
        double differenceHourMinute = Math.abs(TimeUtils.convertToMilli(c) - TimeUtils.convertToMilli(calendar));
        int[] t = TimeUtils.convertMilliInt((long)differenceHourMinute);
    ...
    public String getReadableDifference(Calendar c){
        ...
        if(r[0] != 0 || r[1] != 0 || r[2] != 0){
            ...
            return Date.getReadable(this.calendar) ;
        }else{
...
//        return b;
    ...
    public boolean equals(Object o) {
        ...
        Date date = (Date) o;
        return this.calendar.get(Calendar.YEAR) == date.calendar.get(Calendar.YEAR)
                && this.calendar.get(Calendar.MONTH) == date.calendar.get(Calendar.MONTH)
                && this.calendar.get(Calendar.DATE) == date.calendar.get(Calendar.DATE)
                && this.calendar.get(Calendar.HOUR_OF_DAY) == date.calendar.get(Calendar.HOUR_OF_DAY)
                && this.calendar.get(Calendar.MINUTE) == date.calendar.get(Calendar.MINUTE);
    }
