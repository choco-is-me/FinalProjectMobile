...
public class AlarmClockRecyclerAdapter extends
        MoSelectableAdapter<AlarmClockRecyclerAdapter.MoAlarmClockViewHolder, AlarmClock> {

    public static String UPDATE_ENABLED_VIEW = "update_enabled";

    private MoOnActiveClockChanged listener;

    public AlarmClockRecyclerAdapter(Context c, List<AlarmClock> dataSet, MoOnActiveClockChanged onActiveClockChanged) {
        super(c, dataSet);
    ...
    public void onBindViewHolder(@NonNull MoAlarmClockViewHolder holder, int position) {
        AlarmClock clock = dataSet.get(position);
        clock.setListener(new AlarmClock.MoAlarmClockListener() {
            @Override
    ...
    private void onBindAgain(@NonNull MoAlarmClockViewHolder holder, AlarmClock clock) {
        holder.bind(clock);
    ...
    public static class MoAlarmClockViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(AlarmClock clock) {
        ...
        private void bindRepeatingDays(AlarmClock clock) {
        ...
        private void bindTitle(AlarmClock clock) {
        ...
        private void updateActiveColors(boolean isActive) {
            ...
            TextInput.setColorCondition(isActive, Color.color_text_on_normal, Color.color_text_disabled, title, c);
        }

        private void initEnabledSwitch(AlarmClock clock) {
            enabled.setChecked(clock.isActive());
            enabled.setOnCheckedChangeListener(null);
        }

        private void onSwitchClicked(AlarmClock clock) {
            ...
            if (isActive) {
                ...
                try {
                    AlarmClockManager.getInstance().cancelAlarm(clock.getId(), itemView.getContext());
    ...
    public interface MoOnActiveClockChanged {
        void onActiveStatusChanged(AlarmClock clock);
        void onCardClicked(AlarmClock clock);
    }
}
